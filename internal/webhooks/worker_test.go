...
type MarkRec struct{ ID string; Success bool; Code, Latency int; LastErr string }
type FailRec struct{ ID string; Code, Latency int; LastErr string }
...
func (r *recordStore) MarkWebhookDelivery(ctx context.Context, id string, success bool, nextAttemptAt *time.Time, lastError string, responseCode int, latencyMs int) error {
    r.mu.Lock(); r.marks = append(r.marks, MarkRec{ID: id, Success: success, Code: responseCode, Latency: latencyMs, LastErr: lastError}); r.mu.Unlock()
...
func (r *recordStore) FailWebhookDelivery(ctx context.Context, id string, lastError string, responseCode int, latencyMs int) error {
    r.mu.Lock(); r.fails = append(r.fails, FailRec{ID: id, Code: responseCode, Latency: latencyMs, LastErr: lastError}); r.mu.Unlock()
...
func TestWorkerProcessOnce_SuccessAndSignature(t *testing.T) {
    ...
    id, err := rs.EnqueueWebhook(context.Background(), "t1", "", "stop.advanced", srv.URL, "secret", []byte(`{"id":"evt1"}`))
    if err != nil || id == "" { t.Fatalf("enqueue failed: %v", err) }

    w.processOnce()

    if gotSig == "" || gotType != "stop.advanced" { t.Fatalf("missing signature/type headers: sig=%q type=%q", gotSig, gotType) }
    if len(rs.marks) == 0 || !rs.marks[0].Success { t.Fatalf("expected mark success, got: %+v", rs.marks) }
}
...
func TestWorkerProcessOnce_Fail(t *testing.T) {
    ...
    _, _ = rs.EnqueueWebhook(context.Background(), "t1", "", "stop.advanced", srv.URL, "", []byte(`{}`))
    w.processOnce()
    if len(rs.fails) == 0 { t.Fatalf("expected fail recorded") }
}
