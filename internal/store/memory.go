...
func (m *Memory) PlanRoutes(ctx context.Context, req model.OptimizeRequest) ([]model.Route, string, error) {
	...
	if m.planMx[req.TenantID] == nil {
		m.planMx[req.TenantID] = map[string][]map[string]any{}
	}
...
func (m *Memory) UpdateHOS(ctx context.Context, tenantID, driverID string, upd model.HOSUpdate) (string, map[string]any, error) {
	...
	st := m.hos[driverID]
	if st == nil {
		st = map[string]any{"status": "off", "break": false}
		m.hos[driverID] = st
	}
	...
	case "shift_start":
		status = "on"
		st["shiftStart"] = upd.TS
	case "shift_end":
		status = "off"
		st["shiftEnd"] = upd.TS
	case "break_start":
		st["break"] = true
		st["breakType"] = upd.Type
		st["breakStart"] = upd.TS
	case "break_end":
		st["break"] = false
		st["breakEnd"] = upd.TS
	}
...
func (m *Memory) GetHOS(ctx context.Context, tenantID, driverID string) (string, map[string]any, error) {
	m.mu.Lock()
	defer m.mu.Unlock()
	st := m.hos[driverID]
	if st == nil {
...
func (m *Memory) GetGeofence(ctx context.Context, tenantID, id string) (model.Geofence, error) {
	...
	defer m.mu.Unlock()
	gf, ok := m.gfs[id]
	if !ok || gf.TenantID != tenantID {
		return model.Geofence{}, ErrNotFound
	}
	return gf, nil
}
...
func (m *Memory) MarkWebhookDelivery(ctx context.Context, id string, success bool, nextAttemptAt *time.Time, lastError string, responseCode int, latencyMs int) error {
	...
	if success {
		...
		if nextAttemptAt != nil {
			...
			d.NextAttemptAt = time.Now().Add(1 * time.Minute)
		}
	}
	return nil
}
...
func (m *Memory) RequeueWebhookDLQBulk(ctx context.Context, tenantID string, ids []string) error {
	return nil
}
func (m *Memory) DeleteWebhookDLQBulk(ctx context.Context, tenantID string, ids []string, olderThan time.Time) error {
	return nil
}
...
func (m *Memory) SavePlanMetricsWeights(ctx context.Context, tenantID, planDate, algo string, snaps []map[string]any) error {
	return nil
}
func (m *Memory) ListPlanMetricsWeights(ctx context.Context, tenantID, planDate, algo string) ([]map[string]any, error) {
...
func (m *Memory) GetOptimizerConfig(ctx context.Context, tenantID string) (map[string]any, error) {
	m.mu.Lock()
	defer m.mu.Unlock()
	if cfg, ok := m.optCfg[tenantID]; ok {
		return cfg, nil
	}
	return nil, nil
}
...
func (m *Memory) ListActiveRoutesForDriver(ctx context.Context, tenantID, driverID string) ([]string, error) {
	...
	for id, r := range m.routes {
		if r.DriverID == driverID && r.Status != "completed" {
			out = append(out, id)
		}
	}
	return out, nil
...
func (m *Memory) WebhookMetrics(ctx context.Context, tenantID string, since time.Time, eventType, status string, codeMin, codeMax int, buckets []int) ([]map[string]any, error) {
	...
	add := func(typ, st string, latency int) {
		...
		a := by[key]
		if a == nil {
			a = &agg{b: make([]int, len(buckets)+1)}
			by[key] = a
		}
		a.cnt++
		if latency > 0 {
	...
	for _, ids := range m.deliveriesByTenant {
		for _, id := range ids {
			...
			if !since.IsZero() && d.DeliveredAt != nil && d.DeliveredAt.Before(since) {
				continue
			}
			...
			if codeMin > 0 && d.ResponseCode < codeMin {
				continue
			}
			if codeMax > 0 && d.ResponseCode > codeMax {
				continue
			}
...
// Mapping helpers
func (m *Memory) ListRoutePath(ctx context.Context, tenantID, routeID string) ([]model.GeoPoint, error) {
	m.mu.Lock()
	defer m.mu.Unlock()
	r, ok := m.routes[routeID]
	if !ok {
	...
	// Synthesize a simple polyline near a deterministic base derived from routeID.
	// This makes memory-mode demos render paths without real geometry.
	// Base around SF; add small, deterministic offsets per route.
	baseLat, baseLng := 37.7749, -122.4194
	// crude hash: sum of bytes modulo range
	var sum int
	for i := 0; i < len(routeID); i++ {
		sum += int(routeID[i])
	}
	latOff := (float64(sum%100) - 50.0) * 0.0002
	lngOff := (float64(sum%100) - 50.0) * 0.0002
	lat := baseLat + latOff
	lng := baseLng + lngOff
	// Step size per leg
	dLat := 0.0012
	dLng := 0.0016
	// Start point
	pts := []model.GeoPoint{{Lat: lat, Lng: lng}}
	dir := 1.0
	if (sum/7)%2 == 0 {
		dir = -1.0
	}
	for i := range r.Legs {
		// Alternate direction to make a gentle zig-zag
		if i%2 == 0 {
			lat += dir * dLat
			lng += dLng
		} else {
		...
		pts = append(pts, model.GeoPoint{Lat: lat, Lng: lng})
	}
	return pts, nil
}
