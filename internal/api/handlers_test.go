...
func TestOrdersCreateList(t *testing.T) {
	...
	if rr.Code != http.StatusAccepted {
		t.Fatalf("orders create: got %d", rr.Code)
	}
	// GET /v1/orders
	rr = httptest.NewRecorder()
	...
	s.OrdersHandler(rr, req)
	if rr.Code != 200 {
		t.Fatalf("orders list: got %d", rr.Code)
	}
}
...
func TestRoutesIndexAndGraphQL(t *testing.T) {
	...
	if rr.Code != 200 {
		t.Fatalf("routes index: %d", rr.Code)
	}
	...
	if rr.Code != 200 {
		t.Fatalf("graphql routes: %d", rr.Code)
	}
	...
	if err := json.Unmarshal(rr2.Body.Bytes(), &idx); err == nil && len(idx.Items) > 0 {
		...
		s.GraphQLHTTPHandler(rr, req)
		if rr.Code != 200 {
			t.Fatalf("graphql route: %d", rr.Code)
		}
	}
}
...
func TestDriverRoutesList(t *testing.T) {
	...
	if rr.Code != 200 {
		t.Fatalf("list driver routes: %d", rr.Code)
	}
}
...
func TestAssignAdvanceEnqueuesWebhook(t *testing.T) {
	...
	if err := json.Unmarshal(rr.Body.Bytes(), &ores); err != nil {
		t.Fatalf("decode optimize: %v", err)
	}
	if len(ores.Routes) == 0 {
