(() => {
  ...
  function subscribeRoute(routeId) {
    ...
    (async () => {
      try {
        ...
        for (const it of items) { upsertDriverMarker(it.driverId, it.lat, it.lng); knownDrivers.add(it.driverId); }
        if (typeof refreshFollowList === 'function') refreshFollowList();
      } catch (e) { /* ignore */ }
    })();
    const hdrs = headers();
    ...
    sse.addEventListener('driver.location', (ev) => {
      try {
        ...
        knownDrivers.add(id); if (typeof refreshFollowList === 'function') refreshFollowList();
        if ($('follow').checked) {
  ...
  function colorForId(id){ let h=0; for (let i=0;i<id.length;i++){ h=(h*33 + id.charCodeAt(i))>>>0; } return palette[h % palette.length]; }
  function upsertDriverMarker(id, lat, lng){
    ...
    const ll = [lat, lng];
    if (!driverMarkers[id]) { driverMarkers[id] = L.marker(ll, { title: id, icon }).addTo(driverLayer); }
    else { driverMarkers[id].setLatLng(ll); driverMarkers[id].setIcon(icon); }
  }
  ...
  function refreshFollowList(){
    ...
    // re-render markers to reflect selected styles
