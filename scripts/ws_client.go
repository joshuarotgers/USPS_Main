...
func main() {
    ...
    var optResp struct{ Routes []struct{ ID string `json:"id"` } `json:"routes"` }
    if err := json.NewDecoder(resp.Body).Decode(&optResp); err != nil { log.Fatal(err) }
    if len(optResp.Routes) == 0 { log.Fatal("no routes returned") }
    ...
    // connection_init
    if err := c.WriteJSON(wsMessage{Type: "connection_init"}); err != nil { log.Fatal(err) }
    ...
    if err := c.WriteJSON(wsMessage{Type: "subscribe", ID: "1", Payload: pl}); err != nil { log.Fatal(err) }
    ...
    go func() {
        ...
        for {
            var m wsMessage
            if err := c.ReadJSON(&m); err != nil { log.Printf("read: %v", err); return }
